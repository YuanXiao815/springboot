 ...
 (function($){
	...
	$.extend($.mkPageCalculator.prototype, {
	...
	$.mkPageRenderers.defaultRenderer = function(maxentries, opts) {
		this.maxentries = maxentries;
		this.opts = opts;
		this.pc = new $.mkPageCalculator(maxentries, opts);
	}
	
	$.extend($.mkPageRenderers.defaultRenderer.prototype, {
		...
		getLinks:function(current_page, eventHandler,psize) {
			...
			if(this.opts.showSelect)fragment.append("<select class='tm_psize_go'><option value='5'>5</option><option value='10'>10</option><option value='15'>15</option><option value='20'>20</option><option value='30'>30</option><option value='40'>40</option><option value='50'>50</option></select>&nbsp;");
	...
	// Extend jQuery
	$.fn.mkPage = function(maxentries, opts){
		...
		function mkPageClickHandler(evt){
			var links, 
		...
		function selectPage(new_current_page) {
			...
			links = renderer.getLinks(new_current_page, mkPageClickHandler);
			containers.empty();
		...
		function selectPage_psize(new_current_page,psize) {
			// update the link display of a all containers
			containers.data('current_page', new_current_page);
			links = renderer.getLinks(new_current_page, mkPageClickHandler,psize);
			containers.empty();
			links.appendTo(containers);
		...
		if(!$.mkPageRenderers[opts.renderer])
		{
			throw new ReferenceError("mkPage renderer '" + opts.renderer + "' was not found in jQuery.mkPageRenderers object.");
		}
		renderer = new $.mkPageRenderers[opts.renderer](maxentries, opts);
		
		// Attach control events to the DOM elements
		var pc = new $.mkPageCalculator(maxentries, opts);
		var np = pc.numPages();
		if(np==0 || np==1){
			containers.hide();
		}else{
		...
		links = renderer.getLinks(current_page, mkPageClickHandler);
		containers.empty();
		links.appendTo(containers);
